...
def _plan_for_task(task_id: str) -> List[Action]:
    """
    A simple deterministic "baseline agent":
    - views events
    - performs the minimal correct operations
    - for Task 2, intentionally attempts a duplicate registration once (to demonstrate penalties)
    """
...
def run_baseline_episode(env: CollegeEventEnv, task_id: str) -> Dict[str, Any]:
    """
    Runs the baseline directly against an environment instance (no HTTP).
    ...
    # Ask the grader using the JSON files currently written by the environment.
...
def run_baseline_over_http(
    *,
    ...
    """
    Runs the baseline through HTTP endpoints:
    - POST /env/reset
    - POST /env/step
    - POST /grader
    """
